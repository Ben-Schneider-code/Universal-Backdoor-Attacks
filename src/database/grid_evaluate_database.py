...
class GridEvaluateDatabase:
    """ A wrapper around a pandas dataframe.
    """

    def __init__(self, grid_evaluate_args: 'GridEvaluateArgs'):
        self.grid_evaluate_args = grid_evaluate_args

        self.df = None
        if self.grid_evaluate_args.database_name is not None:
            fp = os.path.abspath(os.path.join(system_configs.CACHE_DIR, self.grid_evaluate_args.database_name))
            if os.path.exists(fp):
                ...
                self.df = self.df[self.df[BaseObserver.ARGS].apply(
                    lambda x: DefenseArgs(**json.loads(x)) in self.grid_evaluate_args.get_all_settings())]
    ...
    def commit(self, df):
        ...
        self.df = df
        if self.grid_evaluate_args.database_name is not None:
            fp = os.path.join(system_configs.CACHE_DIR, self.grid_evaluate_args.database_name)
    ...
    def count(self, defense_args: DefenseArgs) -> int:
        ...
        if self.df is None or len(self.df) == 0:
            return 0
        return self.df.groupby(BaseObserver.ID).last()[BaseObserver.ARGS].apply(
            lambda x: DefenseArgs(**json.loads(x)) == defense_args).tolist().count(True)
    ...
    def filter_remaining(self, settings: List[DefenseArgs]) -> List[DefenseArgs]:
        ...
        for setting in unique_list:
            filtered_settings += [setting] * max(0, self.grid_evaluate_args.repetitions - self.count(setting))
