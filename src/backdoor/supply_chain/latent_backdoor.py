...
class LatentBackdoor(Backdoor):
    """ LatentBackdoor entangles the latent space by minimizing the latent similarity
    between poisoned and clean target classes.
    https://people.cs.uchicago.edu/~ravenben/publications/pdf/pbackdoor-ccs19.pdf
    """

    def train_from_scratch(self, trainer_args: TrainerArgs, model: Model, ds_train: Dataset, ds_test: Dataset):
        trainer = EntangledBackdoorTrainer(trainer_args=trainer_args, env_args=self.env_args)
