...
class NeuralCleanseDetector(Defense):
    """
    ...
    def apply(self, model: Model, ds_train: Dataset = None,
        ...
        for candidate_target in candidate_targets:
            trigger = (torch.rand(ds_train.shape()).to(self.env_args.device) - 0.5) * 0.01
        ...
        print(f"Trigger Norms: {trigger_norms}")
        min_trigger = min(trigger_norms.values())
        return {"model": model.eval(), "score": min_trigger}
